...
yy = zeros(size(y),num_labels); %labels
for i=1:size(X)
  yy(i,y(i)) = 1;
end
...
J = sum((1/m) * sum(-yy .* log(h) - (1 - yy) .* log(1 - h))); 
...
% not vectorized
% for c = 1:num_labels
%  h = sigmoid([ones(length(X),1), X] * Theta1');
%  h = [ones(length(h),1), h];
%  h = sigmoid(h * Theta2(c,:)');
%  
%  J = J + (1/m) * sum(-(y == c)' * log(h) - (1 - (y == c)') * log(1 - h));  
%end
%
% Regularization
%J = J + (lambda/(2*m)) * (sum(sum(Theta1(:,2:end) .^2)) + sum(sum(Theta2(:,2:end).^2)));
